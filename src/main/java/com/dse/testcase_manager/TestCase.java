...
public class TestCase extends AbstractTestCase implements IDataTestItem {
    ...
    private Map<ValueDataNode, ValueDataNode> lambdaInputExpOutputMap = new HashMap<>();
    ...
    public TestCase(ICommonFunctionNode functionNode, String name) {
        ...
        if (functionNode instanceof LambdaFunctionNode) {
            LambdaTypeResolve.getUndefinedFunctionCallLambdaVariables((LambdaFunctionNode) functionNode);
        }
    }
...
//            RootDataNode root = getRootDataNode();
    ...
    public void initLambdaInputExpOutputMap() {
        try {
            lambdaInputExpOutputMap.clear();
            FunctionDetailTree functionDetailTree = new FunctionDetailTree(functionNode);
            DataTree dataTree = new DataTree(functionDetailTree);
            RootDataNode root = dataTree.getRoot();

            // children of the lambda data node of the root are used to be expected output values
            RootDataNode newLambdaDataNode = getLambdaDataNode(root);
            LambdaRootDataNode lambdaDataNode = getLambdaDataNode(rootDataNode);
            if (newLambdaDataNode != null && lambdaDataNode != null) {
                mapLambdaInputToExpOutput(lambdaDataNode.getChildren(), newLambdaDataNode.getChildren());
                lambdaDataNode.setLambdaInputExpOutputMap(lambdaInputExpOutputMap);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    private LambdaRootDataNode getLambdaDataNode(RootDataNode root) {
        for (IDataNode uut : root.getChildren()) {
            if (uut instanceof UnitUnderTestNode) {
                for (IDataNode dataNode : uut.getChildren()) {
                    if (dataNode instanceof LambdaRootDataNode
                            && ((RootDataNode) dataNode).getLevel().equals(NodeType.LAMBDA)) {
                        return (LambdaRootDataNode) dataNode;
                    }
                }
            }
        }
    ...
    private void mapLambdaInputToExpOutput(List<IDataNode> inputs, List<IDataNode> expOutputs) {
        if (inputs != null) {
            for (IDataNode inputValue : inputs) {
                for (IDataNode expectedOutput : expOutputs) {
                    if (expectedOutput.getName().equals(inputValue.getName())) {
                        ((ValueDataNode) expectedOutput).setExternel(false);
                        expectedOutput.setParent(inputValue.getParent());
                        lambdaInputExpOutputMap.put((ValueDataNode) inputValue, (ValueDataNode) expectedOutput);
                    }
                }
            }
        }
