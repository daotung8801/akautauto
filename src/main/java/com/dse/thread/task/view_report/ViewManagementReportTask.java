...
import com.dse.report.FuncTestCaseManagementReport;
...
public class ViewManagementReportTask extends AbstractViewReportTask {
    ...
    protected IReport call() throws Exception {
        IReport report;
        if (nodes.size() == 1 && nodes.get(0) instanceof TestNormalSubprogramNode)
            report = new FuncTestCaseManagementReport(nodes.get(0), LocalDateTime.now());
        else
            report = new TestCaseManagementReport(nodes, LocalDateTime.now());
