...
//    }
    ...
    public static LambdaRootDataNode findLambdaRoot(RootDataNode root) {
        if (root == null)
            return null;
        ...
        if (functionNode != null) {
            ...
            for (IDataNode child : root.getChildren()) {
                if (child instanceof UnitNode && ((UnitNode) child).getSourceNode().equals(source)) {
                    for (IDataNode lambdaRoot : child.getChildren()) {
                        if (lambdaRoot instanceof LambdaRootDataNode
                                && ((RootDataNode) lambdaRoot).getLevel() == NodeType.LAMBDA) {
                            return (LambdaRootDataNode) lambdaRoot;
                        }
                    }
                }
            }
