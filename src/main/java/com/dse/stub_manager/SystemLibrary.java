...
public class SystemLibrary {
    ...
    public static void generateStubCode() {
        // Clear library directory before generating code
        Utils.deleteFileOrFolder(new File(getLibrariesDirectory()));
    ...
    private static String generateFunctionSourceCode(ICommonFunctionNode function) {
        ...
        String functionName = SourceConstant.STUB_PREFIX + function.getSimpleName();

        function.setNameForArgsWithNoName();
        List<Node> parameterNodes = function.getChildren();

        StringBuilder functionParamsString = new StringBuilder();
        for (Node param : parameterNodes) {
            if (param instanceof IVariableNode) {
                String paramType = ((IVariableNode) param).getRawType();
                // Assume that all parameters are named
                String paramName = param.getName();
                // If parameter is a function pointer, its name is already in its type
                if (VariableTypeUtils.isFunctionPointer((paramType))) {
                    functionParamsString.append(String.format("%s, ", paramType));
                } else {
                    functionParamsString.append(String.format("%s %s, ", paramType, paramName));
                }
            }
        }
        ...
        if (functionParamsString.length() > 0) {
            functionParamsString = new StringBuilder(functionParamsString.substring(0, functionParamsString.length() - 2));
        }
    ...
    private static String generateFunctionCall(ICommonFunctionNode functionNode) {
        functionNode.setNameForArgsWithNoName();
        String functionCall = "";
