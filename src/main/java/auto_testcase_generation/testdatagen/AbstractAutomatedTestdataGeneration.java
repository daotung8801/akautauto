...
//                    logger.debug(node.getChildren().get(i).getName() + " is removed");
    ...
    protected List<RandomValue> generateRandomValuesForLambda(RootDataNode root, ICommonFunctionNode sut, TestCase testCase) {
        List<RandomValue> values = new ArrayList<>();
        try {
            LambdaRootDataNode lambdaRoot = Search2.findLambdaRoot(root);

            List<ValueDataNode> lambdaDataNodes = new ArrayList<>();
            for (IDataNode child : lambdaRoot.getChildren()) {
                // not the instance
                if (!(child.getName().startsWith(SourceConstant.INSTANCE_VARIABLE))) {
                    if (child instanceof ValueDataNode) {
                        IVariableNode v = ((ValueDataNode) child).getCorrespondingVar();
//                        if (lambdaRoot.isRelatedVariable(v)) {
                            lambdaDataNodes.add((ValueDataNode) child);
//                        }
                    }
                }
            }
            logger.debug("Num of lambda data nodes = " + lambdaDataNodes);

            for (ValueDataNode lambdaDataNode : lambdaDataNodes) {
                List<IVariableNode> vars = new ArrayList<>();
                vars.add(lambdaDataNode.getCorrespondingVar());
                ...
                // Ex: "Algorithm::Utils::a", name var = "Utils::a" --> prefix = "Algorithm::"
                String prefixName = lambdaDataNode.getVituralName();
                prefixName = prefixName.replaceAll(lambdaDataNode.getName() + "\\s*$", "");

                values.addAll(randomInputGeneration
                        .constructRandomInput(vars, sut.getFunctionConfig(), prefixName));
                recursiveExpandUutBranch(lambdaDataNode, values, testCase);
            }
...
//            String newTestcaseName = nameofTestcase;
            ...
            if (testCase != null) {
                ...
                logger.debug("initLambdaInputExpOutputMap");
                testCase.initLambdaInputExpOutputMap();
            }
