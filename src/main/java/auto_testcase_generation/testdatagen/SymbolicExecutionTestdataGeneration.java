...
public abstract class SymbolicExecutionTestdataGeneration extends AbstractAutomatedTestdataGeneration {
    ...
    protected int iterateDirectly(TestCase testCase, TestcaseExecution executor, ICommonFunctionNode fn,
        ...
        try {
            ...
            if (fn instanceof LambdaFunctionNode && fn.getParent() instanceof ClassNode && randomValuesForInstance != null) {
                theNextTestdata.addAll(randomValuesForInstance);
            }
            List<RandomValue> randomValuesForLambdaVariables = generateRandomValuesForLambda(root, sut, testCase);
            if (fn instanceof LambdaFunctionNode && fn.getParent() instanceof ClassNode && randomValuesForInstance != null) {
                theNextTestdata.addAll(randomValuesForLambdaVariables);
            }
